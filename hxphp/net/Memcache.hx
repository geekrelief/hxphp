package hxphp.net;
import hxphp.net.externs.Memcache;
...
class Memcache {
    ...
    public function get(_key:String):String {
        var val = untyped __php__("$this->m_mc->get($this->prefix.$_key, $this->flags)");
        return cast(val);
    }

    public function getArray(_keys:Array<String>):Array<String> {
        ...
        var val:php.NativeArray = untyped __php__("$this->m_mc->get($keys, $this->flags)");
